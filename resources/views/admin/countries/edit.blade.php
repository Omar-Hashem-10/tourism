...
@section('title', __('admin.edit') . ' ' . __('admin.country_label'))
@section('page-title', __('admin.edit') . ' ' . __('admin.country_label'))

@section('content')
<div class="admin-page-header">
    <div class="admin-page-title">{{ __('admin.edit') }}: {{ $country->getTranslation('name', app()->getLocale()) }}</div>
            ...
            <div class="admin-card">
                ...
                <div style="padding:1.25rem; display:grid; grid-template-columns:1fr 1fr; gap:1rem;">
                    <div>
                        ...
                        <input type="text" name="name[ar]" class="admin-input"
                               value="{{ old('name.ar', $country->getTranslation('name','ar')) }}" required>
                    </div>
                    <div>
                        <label class="admin-label">{{ __('admin.title_en') }}</label>
                        <input type="text" name="name[en]" class="admin-input" style="direction:ltr;"
                               value="{{ old('name.en', $country->getTranslation('name','en')) }}" required>
                    </div>
                </div>
            </div>
        </div>
